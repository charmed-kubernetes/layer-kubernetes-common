...
def test_configure_kubelet(f_open, hookenv, conf_service, get_version, version, tmp_path):
    get_version.return_value = version
    endpoint_from_flag("endpoint.container-runtime.available").get_runtime.return_value = "remote"
...
def test_configure_kubelet_invalid_runtime(hookenv, get_version, tmp_path):
    ...
    endpoint_from_flag("endpoint.container-runtime.available").get_runtime.return_value = "local"
    with pytest.raises(ValueError) as ie:
        kc.configure_kubelet(".test.domain", "10.10.10.10", "registry.k8s.io")
